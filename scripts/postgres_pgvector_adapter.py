...
def verify_m11_r1_hour() -> dict:
    """Inspect the latest V2 H1 context unit; no caller input."""
    relative = "sql/m11_gdelt_v2_verify.sql"
    digest = hashlib.sha256((ROOT / relative).read_bytes()).hexdigest()
    body = f'''file="{REMOTE_FOREX}/{relative}"; test -f "$file"; test "$(sha256sum "$file" | awk '{{print $1}}')" = "{digest}"; docker compose exec -T postgres psql -X -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -At < "$file"'''
...
def stage_m20_lifecycle_summary_query() -> dict:
    ...
    source_windows = subprocess.run(["wslpath", "-w", str(ROOT / relative)], text=True, capture_output=True, check=True).stdout.strip()
    staging_directory = r"C:\Users\chris\Documents\Code\forex-m1-probe"
    staging_windows = staging_directory + r"\m20_lifecycle_summary.sql"
    quote = lambda value: "'" + value.replace("'", "''") + "'"
    mkdir = subprocess.run(build_ssh_command(TARGET, "$ErrorActionPreference='Stop'; New-Item -ItemType Directory -Force -Path " + quote(staging_directory) + " | Out-Null", SETTINGS), text=True, capture_output=True, check=False)
    if mkdir.returncode:
        return wrap("forex_m20_stage_lifecycle_summary_query", {"exit_code": mkdir.returncode, "stdout": mkdir.stdout, "stderr": mkdir.stderr, "ok": False}, digest)
    ...
    transfer = subprocess.run(["powershell.exe", "-NoProfile", "-NonInteractive", "-EncodedCommand", encoded], text=True, capture_output=True, check=False)
    if transfer.returncode:
        return wrap("forex_m20_stage_lifecycle_summary_query", {"exit_code": transfer.returncode, "stdout": transfer.stdout, "stderr": transfer.stderr, "ok": False}, digest)
    body = f'''source="/mnt/c/Users/chris/Documents/Code/forex-m1-probe/m20_lifecycle_summary.sql"
file="{REMOTE_FOREX}/{relative}"
test -f "$source" && [[ "$(sha256sum "$source" | head -c 64)" == "{digest}" ]]
mkdir -p "$(dirname "$file")"
install -m 0644 "$source" "$file"
[[ "$(sha256sum "$file" | head -c 64)" == "{digest}" ]]
printf 'FOREX_M20_LIFECYCLE_SUMMARY_QUERY_STAGED sha256:{digest}\n' '''
    return wrap("forex_m20_stage_lifecycle_summary_query", remote(body), digest)
