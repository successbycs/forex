...
(b/'manifest.json').write_text(json.dumps({'schema_version':'1.0.0','milestone_id':'M3','captured_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'git_revision':(b/'revision.txt').read_text().strip(),'surface':'catalog-locked T480-to-Forex historical MT5 bridge','summary':'FOREX_M3_PROOF_OK','artifacts':files},indent=2)+'\n')
PY
echo "$bundle"
