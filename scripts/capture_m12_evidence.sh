...
m={'schema_version':'1.0.0','milestone_id':'M12','captured_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'git_revision':(b/'revision.txt').read_text().strip(),'dirty_worktree':False,'configuration_fingerprint':s['configuration_fingerprint'],'surface':'T480 fixed normalisation, quality and quarantine processing drill','operation':'fixed M12 quality probe','expected_result':'one accepted observation and explicit quarantine results','observed_result':'FOREX_M12_PROOF_OK','exit_code':0,'redactions':['No market, account, order or article data retained.'],'summary':'FOREX_M12_PROOF_OK','artifacts':a}; (b/'manifest.json').write_text(json.dumps(m,indent=2)+'\n')
PY
echo "$bundle"
