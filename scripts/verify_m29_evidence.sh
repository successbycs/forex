...
l=o('postflight-listener.json','m20_listener_status'); q(l.get('running') is True and l.get('state')=='MAINTENANCE_HOLD','listener not held'); a=o('postflight-account.json','m20_demo_account_liquidity'); q(a.get('server')=='GOMarketsMU-Demo' and a.get('currency')=='AUD' and a.get('open_positions')==0,'postflight not flat Demo'); d=o('postflight-diagnostics.json','m20_listener_diagnostics'); bind=d.get('deployment_binding',{}); q(d.get('maintenance_hold_present') is True and d.get('logon_type')=='S4U' and bind.get('observation')=='VALID' and bind.get('configuration_fingerprint')==m['configuration_fingerprint'],'binding invalid'); print('FOREX_M29_PROOF_OK')
PY
