...
assert 'FOREX_REPOSITORY_VERIFICATION_OK' in (b/'repository-verification.txt').read_text()
...
m={'schema_version':'1.0.0','milestone_id':'M16','captured_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'git_revision':(b/'revision.txt').read_text().strip(),'dirty_worktree':False,'configuration_fingerprint':s['configuration_fingerprint'],'surface':'reproducible historical walk-forward evaluation environment under retrospective H1 bar-close availability assumption','operation':'fixed T480 PostgreSQL M16 chronological walk-forward probe','expected_result':'frozen historical M15 baseline versus no-change and deterministic baselines under declared retrospective availability assumption','observed_result':'FOREX_M16_PROOF_OK','exit_code':0,'redactions':['No account, order, live server, or article text retained.'],'summary':'FOREX_M16_PROOF_OK','artifacts':a}
