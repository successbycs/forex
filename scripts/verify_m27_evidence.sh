...
t=json.loads((b/'tick-proof.json').read_text()); outer=json.loads((b/'listener-status.json').read_text()); assert outer['operation']=='m20_listener_status' and outer['approval_required'] is False and outer['approved'] is False and outer['ok'] is True; response=outer['result']; assert response['exit_code']==0 and response['ok'] is True; state=json.loads(response['stdout']); result=state['last_result']; quote=state['quote']; assert state['running'] is True and state['state'] not in {'STALE','STOPPED','STARTUP_FAILED'}; assert quote is None or (quote['server']=='GOMarketsMU-Demo' and quote['symbol']=='EURUSD'); assert result['server']=='GOMarketsMU-Demo' and result['symbol']=='EURUSD' and result['execution']['status']=='NOT_SUBMITTED'; assert t['operation']==outer['operation'] and t['server']==result['server'] and t['symbol']==result['symbol'] and t['quote_server']==(None if quote is None else quote['server']) and t['quote_symbol']==(None if quote is None else quote['symbol']) and t['assessment_captured_at_utc']==result['captured_at_utc'] and t['listener_state']==state['state'] and t['execution_status']==result['execution']['status']; assessment=datetime.fromisoformat(result['captured_at_utc'].replace('Z','+00:00')); bundle_capture=datetime.fromisoformat(m['captured_at'].replace('Z','+00:00')); assert 0<=t['age_seconds']<=15 and 0<=(bundle_capture-assessment).total_seconds()<=15; print('FOREX_M27_EVIDENCE_VERIFIED')
PY
