...
p=json.loads((b/'replay-probe.json').read_text()); out=p['result']['stdout']; assert p['ok'] and 'FOREX_M13_REPLAY_PROBE_OK' in out and 'FOREX_M13_POSTGRES_REPLAY_OK' in out and 'snapshot=m2-m1-eurusd-h1-720' in out and 'bars=720' in out and 'future_records=0' in out; assert 'FOREX_M13_PROOF_OK' in (b/'summary.txt').read_text(); print('FOREX_M13_EVIDENCE_VERIFIED')
PY
