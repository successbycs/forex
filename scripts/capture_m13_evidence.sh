...
b=Path(sys.argv[1]); p=json.loads((b/'replay-probe.json').read_text()); out=p['result']['stdout']; assert p['ok'] and 'FOREX_M13_POSTGRES_REPLAY_OK' in out and 'snapshot=m2-m1-eurusd-h1-720' in out and 'bars=720' in out and 'replay_days=' in out and 'aligned_context_days=1' in out and 'price_lineage_ok=true' in out and 'context_lineage_ok=true' in out and 'future_price_records=0' in out and 'future_context_records=' in out; s=json.loads(subprocess.check_output(['python3','scripts/forex_milestones.py','status','--json'])); (b/'summary.txt').write_text('FOREX_M13_PROOF_OK\n'); a=[{'path':x.name,'sha256':hashlib.sha256(x.read_bytes()).hexdigest()} for x in sorted(b.iterdir()) if x.is_file()]; m={'schema_version':'1.0.0','milestone_id':'M13','captured_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'git_revision':(b/'revision.txt').read_text().strip(),'dirty_worktree':False,'configuration_fingerprint':s['configuration_fingerprint'],'surface':'point-in-time alignment engine and historical replay environment','operation':'fixed T480 PostgreSQL daily alignment and replay query','expected_result':'strict UTC cutoff selects provenance-linked daily replay rows and excludes future records','observed_result':'FOREX_M13_PROOF_OK','exit_code':0,'redactions':['No market, account, order or live data retained.'],'summary':'FOREX_M13_PROOF_OK','artifacts':a}; (b/'manifest.json').write_text(json.dumps(m,indent=2)+'\n')
PY
echo "$bundle"
