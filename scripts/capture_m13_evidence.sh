...
b=Path(sys.argv[1]); p=json.loads((b/'replay-probe.json').read_text()); assert p['ok'] and 'FOREX_M13_REPLAY_PROBE_OK' in p['result']['stdout']; s=json.loads(subprocess.check_output(['python3','scripts/forex_milestones.py','status','--json'])); (b/'summary.txt').write_text('FOREX_M13_PROOF_OK\n'); a=[{'path':x.name,'sha256':hashlib.sha256(x.read_bytes()).hexdigest()} for x in sorted(b.iterdir()) if x.is_file()]; m={'schema_version':'1.0.0','milestone_id':'M13','captured_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'git_revision':(b/'revision.txt').read_text().strip(),'dirty_worktree':False,'configuration_fingerprint':s['configuration_fingerprint'],'surface':'point-in-time alignment engine and historical replay environment','operation':'fixed T480 M13 replay probe','expected_result':'strict UTC cutoff excludes future context','observed_result':'FOREX_M13_PROOF_OK','exit_code':0,'redactions':['No market, account, order or live data retained.'],'summary':'FOREX_M13_PROOF_OK','artifacts':a}; (b/'manifest.json').write_text(json.dumps(m,indent=2)+'\n')
PY
echo "$bundle"
