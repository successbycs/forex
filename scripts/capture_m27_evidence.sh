...
b=Path(sys.argv[1]); outer=json.loads((b/'listener-status.json').read_text()); state=json.loads(outer['result']['stdout']); result=state['last_result']; captured=datetime.fromisoformat(result['captured_at_utc'].replace('Z','+00:00')); age=(datetime.now(timezone.utc)-captured).total_seconds(); assert outer['operation']=='m20_listener_status' and outer['result']['exit_code']==0 and outer['result']['ok'] and state['running'] and state['state'] not in {'STALE','STOPPED','STARTUP_FAILED'} and state['quote']['server']=='GOMarketsMU-Demo' and state['quote']['symbol']=='EURUSD' and result['server']=='GOMarketsMU-Demo' and result['symbol']=='EURUSD' and result['execution']['status']=='NOT_SUBMITTED' and 0<=age<=15; (b/'tick-proof.json').write_text(json.dumps({'operation':outer['operation'],'server':result['server'],'symbol':result['symbol'],'quote_server':state['quote']['server'],'quote_symbol':state['quote']['symbol'],'assessment_captured_at_utc':result['captured_at_utc'],'age_seconds':age,'listener_state':state['state'],'execution_status':result['execution']['status']},indent=2)+'\n')
PY
