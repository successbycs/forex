...
def test_trigger_now_uses_only_the_fixed_t480_local_webhook(monkeypatch):
    class Shared:
        def config(self):
            return {"base_url": "http://127.0.0.1:5678"}

        def shell_quote(self, value):
            return repr(value)

        def execute_remote(self, script):
            assert "POST \"$base_url/webhook/forex-m11-run-now\"" in script
            return {"ok": True}

    monkeypatch.setattr(n8n_forex_adapter, "shared_n8n", Shared)
    result = n8n_forex_adapter.trigger_now()
    assert result["operation"] == "trigger_now"
    assert result["ok"] is True
