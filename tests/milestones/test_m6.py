...
def test_m6_fixed_probe_covers_declared_closed_timeframes_only():
    ...
    assert 'copy_rates_from_pos(SYMBOL, timeframe, 1, count + 32)' in probe
    assert '"bars_encoding": "gzip+base64-json"' in probe
    assert 'int(rate["time"]) < capture_cutoff' in probe
    assert '"quality_label": "CLOSED_OHLC_VALIDATED"' in probe
