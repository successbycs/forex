...
class DemoSessionLease:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_trades is not None and (not 1 <= self.max_trades <= 10):
            raise DemoTradingError("a legacy Demo trade cap must be between one and ten")
        if self.max_open_positions != 1:
            raise DemoTradingError("M20 permits one open position")
